...
/* Hack -- fix moving file */
#define window_flag op_ptr->window_flag
...
static BOOL allow_sounds = FALSE;
...
static term *term_data_link(int i)
{
    ...
    /* Allocate */
    term *newterm = mem_alloc(sizeof(term));
    ...
    switch ([event type])
    {
        case NSKeyDown:
        {
            ...
            switch([event keyCode]) {
                case kVK_Return: ch = '\n'; break;
                case kVK_Escape: ch = 27 /*ESCAPE*/; break;
                case kVK_Tab: ch = '\t'; break;
                case kVK_Delete: ch = '\b'; break;
                case kVK_ANSI_KeypadEnter: ch = 'n'; kp = TRUE; break;
            }
            ...
            if (ch != '\0')
            {
                ...
                if (mc && MODS_INCLUDE_CONTROL(ch)) ch = KTRL(ch);
                if (ms && MODS_INCLUDE_SHIFT(ch) && (ch & 0x60)) ch &= ~0x20;
                Term_keypress(ch);
            }
